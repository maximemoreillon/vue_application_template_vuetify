<template>
  <v-form class="text-center" @submit.prevent="login()">
    ...
    <div v-if="options.password_reset_url" class="mt-5">
      <a :href="options.password_reset_url">Password reset</a>
    </div>

    <v-subheader
      v-if="options.login_hint"
      class="mt-5 login_hint"
      v-html="options.login_hint"
    />
    ...
    login() {
      ...
      axios
        ...
        .then(({ data }) => {
          ...
          if (this.options.jwt_storage === "localStorage")
